/*
  # Fix User Handles Schema

  1. Changes
    - Drop and recreate user_handles table with proper constraints
    - Update handle verification function
    - Add proper RLS policies
    - Fix trigger function for new users
...
CREATE TABLE user_handles (
  ...
  handle text NOT NULL CHECK (handle ~ '^[a-zA-Z0-9][a-zA-Z0-9_-]{2,29}$'),
...
BEGIN
  ...
  -- Update or insert handle
  INSERT INTO user_handles (user_id, handle)
  VALUES (user_id, new_handle)
  ON CONFLICT (user_id) 
  DO UPDATE SET 
